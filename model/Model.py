...
class model(nn.Module):
    ...
    def __init__(self, numOfEntity, numOfRelation, numOfhidden, dataset, ns):
        ...
        if "forecast" in self.dataset:
            self.decay_rate = torch.nn.Parameter(data = torch.cuda.FloatTensor([[0.00]]), requires_grad = False) # ICEWS18, 05, 0 requires_grad = False
        else:
            self.decay_rate = torch.nn.Parameter(data = torch.cuda.FloatTensor([[0.01]]), requires_grad = True)
        self.trade_off = torch.nn.Parameter(data = torch.cuda.FloatTensor(1,1), requires_grad = True) # ICEWS05, 0, requires_grad = False
    ...
    def get_local_loss(self, p_s, p_r, p_o, p_t, n_s, n_o, \
        ...
        s_neg_score = torch.sum(s_neg_score*r_att_s.unsqueeze(1), 3)# + 1#*(time_decay == 1).float() #batch_size*time_window
    ...
    def get_global_loss(self, p_s, p_o, p_t, n_s, n_o, s_h_e_flat, o_h_e_flat, p_s_d, p_o_d, p_t_m, p_s_o_r):
        ...
        p_s_his_embedding = self.get_time_embedding_A(p_s, torch.relu(p_t - 1).unsqueeze(1), 1).unsqueeze(1) #batch_size*1*hidden
        s_his_community = torch.softmax(self.auxiliary_matrix(p_s_his_embedding), dim=2) #batch_size*1*community_num
        p_o_his_embedding = self.get_time_embedding_A(p_o, torch.relu(p_t - 1).unsqueeze(1), 1).unsqueeze(1) #batch_size*1*hidden
        ...
        p_s_community = torch.softmax(self.auxiliary_matrix(p_s_embedding + s_C_embedding), dim=1) #batch_size*community_num
        p_s_community = (self.numOfEntity**0.5 * p_s_community**0.5)/((p_s_community**0.5).sum(dim=1).sum(dim=0))
        p_o_community = torch.softmax(self.auxiliary_matrix(p_o_embedding + o_C_embedding), dim=1) #batch_size*community_num
        p_o_community = (self.numOfEntity**0.5 * p_o_community**0.5)/((p_o_community**0.5).sum(dim=1).sum(dim=0))

        global_loss = - torch.mean(torch.sum(p_s_community*p_o_community, 1)*modularity, 0)
    ...
    def validate(self,p_s, p_r, p_o, p_t, \
        ...
        r_score = torch.sum(self.Transfer_R(p_r_embedding).unsqueeze(1).unsqueeze(1)*s_h_r_embedding, 3) #batch_size*time_window*relation_sum(sample)
        r_att_s = self.masked_softmax(r_score, s_h_r, self.numOfRelation, 2) #batch_size*time_window*relation_sum(sample)
        s_his_score = torch.sum(s_his_score*r_att_s, 2)# + 1#*(time_decay == 1).float() #batch_size*time_window
        ...
        r_score = torch.sum(self.Transfer_R(p_r_embedding).unsqueeze(1).unsqueeze(1)*o_h_r_embedding, 3) #batch_size*time_window*relation_sum(sample)
        r_att_o = self.masked_softmax(r_score, o_h_r, self.numOfRelation, 2) #batch_size*time_window*relation_sum(sample)
        o_his_score = torch.sum(o_his_score*r_att_o, 2)# + 1#*(time_decay == 1).float() #batch_size*time_window
